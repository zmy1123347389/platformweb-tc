...
public class ImageUtils {
    ...
    public static String setFileToModel(FTPConfig ftpConfig, MultipartFile file) {
        String picNewName = "";
        String picHttpAddress = "";
        try {
            if (StringUtils.isNotBlank(file.getOriginalFilename())) {
                String oldName = file.getOriginalFilename();
                picNewName = UploadUtils.generateRandonFileName(oldName);
                picHttpAddress = FTPUtil.pictureUploadByConfig(ftpConfig, picNewName, file.getInputStream());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        return picHttpAddress;
    }
}
